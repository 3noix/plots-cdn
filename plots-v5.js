...
async function main() {
  ...
  const dataTransformer = new data_transformers_1.FdoatDataTransformer({
    data,
    ifParamNotFound: "show-in-legend"
  });
  // @2: for Python/CSV
  // const parametersStr = document.querySelector("#data-parameters")?.textContent ?? "";
  // const parameters = JSON.parse(await gzippedBase64ToUtf8(parametersStr)).map((p: any) => ({...p, values: p.values.map((v: any) => (v == null) ? NaN : v)}));
  // const dataTransformer = new CsvDataTransformer({parameters, timeParamName: "time", ifParamNotFound: "show-in-legend"});
  // @2: layout
