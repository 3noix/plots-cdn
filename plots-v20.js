...
class CsvDataTransformer extends data_transformer_1.DataTransformer {
  ...
  getSyspropForXValue(input) {
    ...
    return this._paramTime.timeFormat === "timestamp-seconds" ? 1000 * input : input;
  }
}
...
class PlotBooleans extends plot_1.Plot {
  ...
  constructor(props) {
    ...
    vlinesConfig.map(vlc => {
      const x = this.dataTransformer.getSyspropForXValue(vlc.x);
      if (isNaN(x)) {
        return null;
      }
      const vline = this.createVline(vlc, x);
      const text = vlc.showLabel ? this.createTextAnnotation({
        text: vlc.label,
        color: vlc.lineColor,
        x,
      ...
      return [vline, text];
    }).filter(x => x != null).forEach(([span, label]) => {
