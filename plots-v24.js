...
class CsvDataTransformer extends data_transformer_1.DataTransformer {
  ...
  constructor(props) {
    ...
    const timeFormat = checkTime(paramTime.values);
    if (timeFormat === "timestamp-seconds") {
      this._paramTime = {
        name: paramTime.name,
        values: paramTime.values.map(t => 1000 * t),
        xAxisType: "datetime",
        timeFormat
      };
    } else if (timeFormat === "timestamp-milliseconds") {
      this._paramTime = {
        ...paramTime,
        xAxisType: "datetime",
        timeFormat
  ...
  xAxisType() {
    return this._paramTime.xAxisType;
  }
  ...
  toColumnDataSourceMap(nameLat, nameLon, nameHdg, refSamples, interpolation, ifParamNotFound) {
    const key = `map/${nameLat}/${nameLon}/${nameHdg}`;
    const cachedCds = this._cache.get(key);
    if (cachedCds != undefined) {
      return cachedCds;
    }
    ...
    const paramHdg = this._params.find(p => p.name === nameHdg);
    ...
    const data = {
      ...
      h: paramHdg == undefined ? paramLon.values.map(x => NaN) : paramHdg.values
    };
    const cds = new Bokeh.ColumnDataSource({
      data
    });
  ...
  getPreviousMapValuesAt(nameLat, nameLon, nameHdg, t) {
    const key = `map/${nameLat}/${nameLon}/${nameHdg}`;
    const cachedCds = this._cache.get(key);
    if (cachedCds == undefined) {
      return null;
    ...
    // @ts-ignore
    const tValues = cachedCds.data["t"];
    // @ts-ignore
    const xValues = cachedCds.data["x"];
    // @ts-ignore
    const yValues = cachedCds.data["y"];
    // @ts-ignore
    const hValues = cachedCds.data["h"];
    const index = (0, utils_1.getPreviousByDichotomy)(tValues, t);
    if (index == null) {
      return null;
    }
    const x = xValues[index];
    const y = yValues[index];
    const h = hValues[index];
  ...
  getSyspropForXValue(input) {
    if (typeof input === "string") {
      return [];
    }
    return this._paramTime.timeFormat === "timestamp-seconds" ? input.map(x => 1000 * x) : input;
  }
}
exports.CsvDataTransformer = CsvDataTransformer;
function checkTime(times) {
  if (times.length === 0) {
    throw new Error("The array of time values cannot be empty");
  }
  const secMin = (2005 - 1970) * 365 * 24 * 3600;
  const secMax = (2050 - 1970) * 365 * 24 * 3600;
  const tStart = times[0];
  const tStop = times[times.length - 1];
  if (tStart > secMin && tStart < secMax && tStop > secMin && tStop < secMax) {
    return "timestamp-seconds";
  }
  if (tStart > 1000 * secMin && tStart < 1000 * secMax && tStop > 1000 * secMin && tStop < 1000 * secMax) {
    return "timestamp-milliseconds";
  }
  if (tStart < 100000 && tStop < 1000000) {
    return "starts-with-zero-seconds";
  }
  return "??";
}
...
class FdoatDataTransformer extends data_transformer_1.DataTransformer {
  ...
  toColumnDataSourceMap(nameLat, nameLon, nameHdg, refSamples, interpolation, ifParamNotFound) {
    ...
    const paramHdg = this._data.parameters.find(p => p.name === nameHdg);
    if (paramLat == undefined) {
      return this.handleParameterNotFound(nameLat, ifParamNotFound);
    }
    if (paramLon == undefined) {
    ...
    const hdgValues = paramHdg == undefined ? latValues.map(x => NaN) : reference === "lon" ? reinterpolateParameter(paramHdg, paramLon, "previous") : reinterpolateParameter(paramHdg, paramLat, "previous");
    const data = {
      t: timeValues,
  ...
  getPreviousMapValuesAt(nameLat, nameLon, nameHdg, t) {
    ...
    // @ts-ignore
    const hValues = cachedCds.data["h"];
    const index = (0, utils_1.getPreviousAssumingEquallySpaced)(tValues, t);
    if (index == null) {
      return null;
    }
    const x = xValues[index];
  ...
  getSyspropForXValue(input) {
    ...
    const sysprop = this._data.properties.find(p => p.name === input);
    if (sysprop == undefined || typeof sysprop.value === "string") {
      return [];
    }
...
function parseStoredConfig(x) {
  ...
  if (typeof x.map.autoZoomMarginFactor !== "number") {
    e(`input.map.autoZoomMarginFactor is not a number`);
  }
  return x;
}
function e(msg) {
...
function readConfigFromLocalStorage() {
  try {
    ...
    exports.g.config.map.autoZoomMarginFactor = config.map.autoZoomMarginFactor;
  ...
  try {
    const configToStore = {
      ...
      map: {
        ...
        autoZoomMarginFactor: exports.g.config.map.autoZoomMarginFactor
      }
    };
...
const defaultAutoZoomMarginFactor = 0.05;
exports.g = {
  config: {
    toolbars: {
    ...
    map: {
      ...
      autoZoomMarginFactor: defaultAutoZoomMarginFactor
    }
  },
  state: {
    mode: "none"
...
const utils_1 = __webpack_require__(/*! ../utils */ "./src/webviews/plots/utils.ts");
function addCrosshair(plots, dataTransformer) {
  const xAxisType = dataTransformer.xAxisType();
  plots.forEach(plot => {
    ...
    const onMouseMoveOrOver = event => {
        ...
        if (plotMode !== "map" && plotMode2 === "map") {
          ...
          plot2.elements.interactibles.crosshair.aircraftSymbols.forEach((acs, i) => {
            ...
            if (!curve.line.visible) {
              acs.point.style.display = "none";
              acs.icon.style.display = "none";
              return;
            }
            const data = dataTransformer.getPreviousMapValuesAt(curve.config.paramName, curve.config.paramNameLon ?? "", curve.config.paramNameTrueHdg ?? "", x);
            if (data == null) {
              acs.point.style.display = "none";
              acs.icon.style.display = "none";
              return;
            ...
            const {
              x: offsetX,
              y: offsetY
            } = (0, utils_1.computePositionInElementReference)(plot2, data.x, data.y);
            const showIcon = !isNaN(data.hdg);
            const rotation = (showIcon ? data.hdg : 0) % 360;
            acs.point.style.display = showIcon ? "none" : "block";
            acs.icon.style.display = showIcon ? "block" : "none";
            acs.point.style.left = `${offsetX}px`;
            acs.icon.style.left = `${offsetX}px`;
            acs.point.style.top = `${offsetY}px`;
            acs.icon.style.top = `${offsetY}px`;
            acs.icon.style.rotate = `${rotation}deg`;
          });
        }
      });
      ...
      if (plotMode !== "map") {
        plot.elements.interactibles.crosshair.xvalue.innerHTML = computeXValueText(x, xAxisType);
...
function computeXValueText(value, xAxisType) {
  if (xAxisType === "linear") {
    const [integerPart, decimalPart] = value.toFixed(2).split(".");
    return `${integerPart}<span style="font-size: 0.8em">.${decimalPart}</span>`;
  } else {
    ...
    const milliseconds = String(Math.floor(0.1 * date.getUTCMilliseconds())).padStart(2, "0");
    return `${date.getFullYear()}-${month}-${day} ${hours}:${minutes}:${seconds}<span style="font-size: 0.8em">.${milliseconds}</span>`;
  }
}
...
function addDndEvents(plots) {
  ...
  function setZoomGuiMode(mode, index) {
    plots.forEach((plot, i) => {
      ...
      if (plot.elements.interactibles.selectionBandX != undefined) {
        plot.elements.interactibles.selectionBandX.style.display = showBandX ? "block" : "none";
      }
      if (plot.elements.interactibles.selectionBandY != undefined) {
        plot.elements.interactibles.selectionBandY.style.display = showBandY ? "block" : "none";
      }
      if (plot.elements.interactibles.selectionRect != undefined) {
  ...
  function setZoomBox(event) {
    ...
    const selectionRect = global_1.g.state.plots[global_1.g.state.index].elements.interactibles.selectionRect;
    if (selectionRect != undefined) {
      selectionRect.style.top = `${top - 1}px`;
      selectionRect.style.bottom = `${bottom - 1}px`;
      selectionRect.style.left = `${left - 1}px`;
      selectionRect.style.right = `${right - 1}px`;
    }
  }
  ...
  function setZoomBandX(event) {
    ...
    global_1.g.state.plots.forEach(plot => {
      ...
      const left = Math.min(global_1.g.state.start.clientX, event.clientX) - rect.left;
      const right = rect.right - Math.max(global_1.g.state.start.clientX, event.clientX);
      const selectionBandX = plot.elements.interactibles.selectionBandX;
      if (selectionBandX != undefined) {
        selectionBandX.style.left = `${left - 1}px`;
        selectionBandX.style.right = `${right - 1}px`;
      }
    });
  }
  ...
  function setZoomBandY(event) {
    ...
    const top = Math.min(global_1.g.state.start.clientY, event.clientY) - rect.top;
    const bottom = rect.bottom - Math.max(global_1.g.state.start.clientY, event.clientY);
    const selectionBandY = global_1.g.state.plots[global_1.g.state.index].elements.interactibles.selectionBandY;
    if (selectionBandY != undefined) {
      selectionBandY.style.top = `${top - 1}px`;
      selectionBandY.style.bottom = `${bottom - 1}px`;
    }
  }
...
function createLegendItem(curve, plot, dataTransformer) {
  ...
  const setValueAt = t => {
    ...
    span2.textContent = y == null ? "" : plotMode === "normal" ? ` = ${y.toFixed(curve.config.nbDigits ?? 2)}${curve.config.legendSuffix ?? ""}` : ` = ${y}`;
  };
  return {
    element: legendItem,
    setValueAt
...
function addMapTilesSelector(plots) {
  plots.forEach(plot => {
    ...
    if (plot.elements.interactibles.mapTilesSelector.element != undefined) {
      plot.elements.interactibles.mapTilesSelector.element.style.display = "block";
    }
    if (plot.elements.interactibles.mapTilesSelector.radios != undefined) {
...
function setAttribution(plot, mapTiles) {
  if (plot.elements.interactibles.mapTilesAttribution == undefined) {
    return;
  }
...
function getAttributionHtml(mapTiles) {
  ...
  } else if (mapTiles === "GeoportailFrance.orthos") {
    return '<a target="_blank" href="https://www.geoportail.gouv.fr/">Geoportail France</a>';
  } else {
    return null;
  }
}
...
function computeMapTilesUrl(mapTiles) {
  ...
  if (mapTiles === "OpenTopoMap") {
    return "https://tile.opentopomap.org/{z}/{x}/{y}.png";
  }
  if (mapTiles === "Esri.WorldImagery") {
    return "https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}";
  }
  if (mapTiles === "GeoportailFrance.orthos") {
...
const e = {
  ...
  map: {
    ...
    autoZoomMarginFactor: document.querySelector("#map-auto-zoom-margin-factor")
  },
  buttons: {
...
function addSettings(plots) {
  e.buttons.open.addEventListener("click", event => {
    ...
    e.map.autoZoomMarginFactor.valueAsNumber = global_1.g.config.map.autoZoomMarginFactor;
    e.dialog.showModal();
  });
  e.buttons.save.addEventListener("click", event => {
    ...
    global_1.g.config.map.autoZoomMarginFactor = e.map.autoZoomMarginFactor.valueAsNumber;
...
exports.defaultMapAutoZoomOnTimeRangeChange = exports.defaultMapTiles = exports.defaultInterpolation = exports.defaultIfParamNotFound = void 0;
...
exports.defaultMapAutoZoomOnTimeRangeChange = false;
...
class PlotBooleans extends plot_1.Plot {
  ...
  constructor(props) {
    ...
    this.fig = new Bokeh.Plotting.Figure({
      ...
      x_axis_type: this.dataTransformer.xAxisType(),
      y_axis_type: "linear"
    });
    this.fig.legend.visible = false;
    this.fig.toolbar.logo = null;
    if (this.dataTransformer.xAxisType() === "datetime") {
      const ticker = new Bokeh.DatetimeTicker({
        desired_num_ticks: 11
      });
      this.fig.xaxis.ticker = ticker;
      this.fig.xgrid.ticker = ticker;
      this.fig.xaxis.formatter = new Bokeh.DatetimeTickFormatter({
        years: "%Y",
        months: "%Y-%m",
        days: "%Y-%m-%d",
        hours: "%Y-%m-%d %H:%M",
    ...
    const vlinesConfig = [...props.global_objects.vlines, ...(props.config.vlines ?? [])];
    vlinesConfig.flatMap(vlc => {
      const xValues = this.dataTransformer.getSyspropForXValue(vlc.x);
      return xValues.map(x => {
        const span = this.createVline(vlc, x);
        const label = vlc.showLabel ? this.createTextAnnotation({
          text: vlc.label,
          color: vlc.lineColor,
          x,
          dy: vlc.offsetVersusTopBottom ?? 5,
          from: vlc.placeLabelVersus ?? "top"
        }) : null;
        return {
          span,
          label
      ...
      this.fig.add_layout(span);
      if (label != null) {
        this.fig.add_layout(label);
      }
    });
    const intervalsConfig = [...props.global_objects.intervals, ...(props.config.intervals ?? [])];
    intervalsConfig.flatMap(ic => {
      const n1 = typeof ic.x1 === "string" ? 1 : ic.x1.length;
      const n2 = typeof ic.x2 === "string" ? 1 : ic.x2.length;
      const n = Math.min(n1, n2);
      const x1Values = this.dataTransformer.getSyspropForXValue(ic.x1).slice(0, n);
      const x2Values = this.dataTransformer.getSyspropForXValue(ic.x2).slice(0, n);
      return x1Values.map((x1, i) => ({
        x1,
        x2: x2Values[i]
      })).map(({
        x1,
        ...
        x1,
        x2,
        fillColor: ic.fillColor ?? "#ff0000",
        fillAlpha: ic.fillAlpha ?? 0.2
      }));
    }).forEach(inter => {
      this.fig.add_layout(inter);
    });
...
class PlotMap extends plot_1.Plot {
  ...
  constructor(props) {
    ...
    this.curves = this.config.curves.map(c => {
      c.interpolation = c.interpolation ?? this.config.interpolation ?? defaults_1.defaultInterpolation;
      const cds = this.dataTransformer.toColumnDataSourceMap(c.paramName, c.paramNameLon ?? "", c.paramNameTrueHdg ?? "", "auto", c.interpolation, c.ifParamNotFound);
      if (cds != null) {
        // @ts-ignore
  ...
  updateTrajectories(t1, t2) {
    ...
    if (this.config.mapAutoZoomOnTimeRangeChange ?? defaults_1.defaultMapAutoZoomOnTimeRangeChange) {
      this.autoZoom(t1, t2);
    }
  }
  autoZoom(t1, t2) {
    const {
      width: windowWidth,
      height: windowHeight
    } = this.elements.frame.getBoundingClientRect();
    if (windowHeight === 0) {
      return false;
    }
    // curve.xrange = minmax(curve.dataSource.data["x"]);
    const xRange = this.curves.filter(c => c.config.disabled !== true).map(c => {
      const data = c.dataSource.data;
      return (0, utils_1.minmax)(data["x"].filter((x, i) => {
        const t = data["t"][i];
        return t >= t1 && t <= t2;
      }));
    }).filter(xr => xr != undefined).reduce((acc, v) => ({
      min: Math.min(acc.min, v.min),
      max: Math.max(acc.max, v.max)
    }), {
      min: Infinity,
    ...
    const yRange = this.curves.filter(c => c.config.disabled !== true).map(c => {
      const data = c.dataSource.data;
      return (0, utils_1.minmax)(data["y"].filter((y, i) => {
        const t = data["t"][i];
        return t >= t1 && t <= t2;
      }));
    }).filter(yr => yr != undefined).reduce((acc, v) => ({
      min: Math.min(acc.min, v.min),
      max: Math.max(acc.max, v.max)
    }), {
      min: Infinity,
    ...
    const cx = 0.5 * (xRange.min + xRange.max);
    const cy = 0.5 * (yRange.min + yRange.max);
    const newXRange = (0, utils_1.addMargin)({
      start: xRange.min,
      end: xRange.max
    }, 0);
    const newYRange = (0, utils_1.addMargin)({
      start: yRange.min,
      end: yRange.max
    }, 0);
    const dxmin = (newXRange.end - newXRange.start) * (1 + 2 * global_1.g.config.map.autoZoomMarginFactor);
    const dymin = (newYRange.end - newYRange.start) * (1 + 2 * global_1.g.config.map.autoZoomMarginFactor);
    const windowRatio = windowWidth / windowHeight;
    const dataTooWide = dxmin / dymin > windowRatio;
    const dx = dataTooWide ? dxmin : dymin * windowRatio;
    const dy = dataTooWide ? dxmin / windowRatio : dymin;
    this.setXRange(cx - 0.5 * dx, cx + 0.5 * dx);
    this.setYRange(cy - 0.5 * dy, cy + 0.5 * dy);
    return true;
  }
}
...
class PlotNormal extends plot_1.Plot {
  ...
  constructor(props) {
    ...
    if (this.config.yTicksLabels != undefined) {
      this.fig.yaxis.ticker = new Bokeh.FixedTicker({
        ticks: this.config.yTicksLabels.map(t => t[0]),
        minor_ticks: []
      });
      this.fig.ygrid.ticker = new Bokeh.FixedTicker({
        ticks: this.config.yTicksLabels.map(t => t[0]),
      ...
      this.fig.yaxis.major_label_overrides = new Map(this.config.yTicksLabels);
    }
    this.fig.yaxis.axis_label_text_font_style = "normal";
    this.fig.yaxis.axis_label_text_font_size = "14px";
...
class Plot {
  ...
  createVline(config, x) {
    return new Bokeh.Span({
      location: x,
      dimension: "height",
      line_color: config.lineColor,
      line_alpha: config.lineAlpha,
  ...
  createTextAnnotation(config) {
    return new Bokeh.Label({
      text: config.text,
      x: config.x,
      y: config.dy,
      x_offset: 5,
      text_color: config.color,
      anchor: config.from === "bottom" ? "bottom_left" : "top_left",
      y_units: config.from === "bottom" ? "screen" : "canvas"
    });
  }
  createInterval(config) {
    if (config.x1 === config.x2) {
      // return new Bokeh.Span({
      // 	location: config.x1,
      // 	dimension: "height",
      // 	line_color: config.fillColor,
      // 	line_alpha: config.fillAlpha,
      // 	line_width: 0.5,
      // 	line_dash: "solid"
      // });
      const delta = this.dataTransformer.xAxisType() === "linear" ? 0.1 : 100; // we add 0.5s to x2
      return new Bokeh.BoxAnnotation({
        left: config.x1,
        right: config.x2 + delta,
        fill_color: config.fillColor,
        fill_alpha: config.fillAlpha,
        line_color: config.fillColor,
        line_alpha: 0.5 * config.fillAlpha,
        line_width: 0.5
      });
    } else {
      return new Bokeh.BoxAnnotation({
  ...
  setElements(elements) {
    ...
    this.elements.spans.forEach(elt => {
      elt.style.pointerEvents = "none";
    });
    this.elements.labels.forEach(elt => {
      elt.style.pointerEvents = "none";
    });
    this.elements.boxAnnotations.forEach(elt => {
      elt.style.pointerEvents = "none";
    });
    if (this.config.mode === "map") {
      ...
      this.getCurves().forEach(c => {
        const aircraftPoint = (0, utils_1.copyTemplate)("aircraft-symbol");
        const aircraftIcon = (0, utils_1.copyTemplate)("aircraft-icon");
        aircraftPoint.style.backgroundColor = c.config.lineColor;
        aircraftIcon.style.backgroundColor = "transparent";
        aircraftIcon.style.color = c.config.lineColor;
        this.elements.frame.shadowRoot?.appendChild(aircraftPoint);
        this.elements.frame.shadowRoot?.appendChild(aircraftIcon);
        this.elements.interactibles.crosshair.aircraftSymbols.push({
          point: aircraftPoint,
          icon: aircraftIcon
        });
      });
...
function getPlotsElements(layout) {
  ...
  return {
    ...
    bkGridPlot,
    bkGridBox,
    bkFigures: bkFigures2.map((elt, i) => getZones(elt, layout.plots[i])).filter(x => x != null)
  };
}
function getZones(plotElement, plotConfig) {
  ...
  const axes = [...divLayers[0].querySelectorAll("div.bk-DatetimeAxis"), ...divLayers[0].querySelectorAll("div.bk-LinearAxis"), ...divLayers[0].querySelectorAll("div.bk-MercatorAxis")];
  ...
  const spans = [...divLayers[0].querySelectorAll("div.bk-Span")];
  const labels = [...divLayers[0].querySelectorAll("div.bk-Label")];
  const boxAnnotations = [...divLayers[0].querySelectorAll("div.bk-BoxAnnotation")];
  ...
  let selectionBandX = undefined;
  let selectionBandY = undefined;
  if (plotConfig.mode !== "map") {
    selectionRect = copyTemplate("selection-rect");
    selectionBandX = copyTemplate("selection-band-x");
    selectionBandY = copyTemplate("selection-band-y");
  ...
  // map tiles
  let mapTilesSelection = undefined;
  let mapTilesAttribution = undefined;
  if (plotConfig.mode === "map") {
    mapTilesSelection = copyTemplate("map-tiles-selection");
    mapTilesAttribution = copyTemplate("map-tiles-attribution");
  ...
  return {
    ...
    interactibles: {
      ...
      mapTilesSelector: {
        ...
        radios: mapTilesSelection == undefined ? undefined : [...mapTilesSelection.querySelectorAll("input")].map(input => ({
          input,
...
document.addEventListener("keydown", event => {
  if (event.key === "F5" || event.key === "x" && event.altKey) {
    reload();
...
async function main() {
  // @2: for FDOAT
  // const dataStr = document.querySelector("#data-parameters")?.textContent ?? "";
  // const data = JSON.parse(await gzippedBase64ToUtf8(dataStr));
  // data.parameters = data.parameters.map((p: any) => ({...p, values: p.values.map((v: any) => (v == null) ? NaN : v)}));
  ...
  const parameters = JSON.parse(await gzippedBase64ToUtf8(parametersStr)).map(p => ({
    ...p,
  ...
  const layout = JSON.parse(base64ToUtf8(layoutStr));
  // @2: main
  ...
  // @2: on load
  function onLoad() {
    const elements = (0, utils_1.getPlotsElements)(layout);
    if (elements == null) {
    ...
    interactions.addCrosshair(plots, dataTransformer);
    ...
    document.addEventListener("keydown", event => {
      if (event.key === "Backspace") {
        const p = plots.find(p => p.getConfig().mode !== "map");
        if (p === undefined) {
          return;
        }
        p.resetXRange();
...
// MARK: decode
async function gzippedBase64ToUtf8(base64str) {
...
function base64ToUtf8(base64str) {
  return new TextDecoder("utf-8").decode(base64ToBytes(base64str));
}
// MARK: build plots
